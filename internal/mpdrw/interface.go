...
type MpdRW interface {
	...
	// - IOError: returned if connection is lost
	// - ACKError: theoretically possible if the client receives an ACK response for the IDLE command; practically, this should not happen.
	...
	// Can return the following errors:
	// - IOError: returned if connection is lost.
	// - ACKError: returned if an ACK response is received from the MPD server.
